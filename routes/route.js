...
//user profile
authController.userProfileUpdate = function (request, response) {
    var data = request.body;
    //console.log(data);
    Model.userProfileUpdate(data, response, function (err, message) {
        if (err) {
            return response.send(err)
        } else {
            return response.redirect('/userProfile');
        }
    })
}
